...
// --------------------------------------------------------------------------------------------------------
	...
	public function getFieldOnId($table_name, $id, $field_out){
		...
		return $this->getFieldOnId($table_name, $field_out, "id", $id);
	}
	...
	public function deleteOnID ($table_name, $id) {
		if (!$this->existsID($table_name, $id)) return false;
		return $this->update($table_name, "`id` = `$id`");
	}
	
	
	public function setFieldId ($table_name, $field, $value, $field_in, $value_in) {
	...
	public function setFieldOnID ($table_name, $id, $field, $value) {
		if (!$this->existsID($table_name, $id)) return false;
		return $this->setFieldId($table_name, $field, $value, "id", $id);
	}
	...
	public function search ($table_name, $words, $fields) {
		...
		for ($i = 0; $i < count($results); $i++) {
			for ($j = 0; $j < count($fields); $j++) {
				$results[$t][$fields[$i]] = mb_strtolower(strip_tags($results[$t][$fields[$i]]));
			}
			$data[$k] = $results[$i];
	...
	public function orderResultSearch ($data, $order) {
